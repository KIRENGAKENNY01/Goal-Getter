...
const ChangePerson:React.FC<{Id:number}> = ({Id}) => {
   ...
   return (
    ...
    <Link to={`/deletePerson/${Id}`} className='flex items-center gap-4 bg-light_button p-2'>
      <img src={Delete} alt="trash icon" className='w-[20%]'/>
      <p className='text-white font-semibold'>Delete Task</p>
      </Link>
      <Link to={`/updatePerson/${Id}`} className='flex gap-4 bg-light_button p-2'>
